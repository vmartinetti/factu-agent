...
import { parseEuropeanNumber } from "./helpers";
 ...
 */
function extractDollarRate(table: cheerio.Cheerio): { date: string; salesRate: string } {
  ...
  const date = `${new Date().getFullYear()}-${month.toString().padStart(2,"0")}-${day}`;
  // const _compra = cells.eq(COMPRA_INDEX).text().trim();
  const salesRate = cells.eq(VENTA_INDEX).text().trim();
  
  return { date, salesRate };
}

/**
 * Main function to retrieve the latest dollar exchange rate
 */
export async function getLatestDollarExchangeRate(): Promise<ExchangeRateResult> {
  try {
    const html = await fetchDnitHtml();
    const $ = cheerio.load(html);
    
    const exchangeTable = findExchangeRateTable($);
    const { date, salesRate } = extractDollarRate(exchangeTable);
    const salesRateNumber = parseEuropeanNumber(salesRate);
    if(!salesRateNumber){
      throw new Error("Invalid sales rate format");
    }
    if (isNaN(salesRateNumber)) {
      throw new Error("Invalid sales rate number");
    }
    return { 
      success: true, 
      data: { date, salesRate: salesRateNumber}, 
    };
  } catch (error) {
